...
class CrawlDbMetasMerger extends MapReduceBase implements  Mapper<Text, CrawlDatum, Text, CrawlDatum> {
	...
	public static final String META = "_Trylog_Pagerank_";
	...
	public void map(Text key, CrawlDatum value, OutputCollector<Text, CrawlDatum> output, Reporter reporter) throws IOException {
		...
		if(modificationsMap.containsKey(url)) {
			...
			Float new_pagerank = modificationsMap.get(url);
			LOG.info("Modifiying " + META  + "-> " + new_pagerank);
			meta.put(new Text(META), new FloatWritable(new_pagerank));
		}
...
public class PageranksUpdateThread  extends TaskThread {
	private static final Logger LOG = Logger.getLogger(PageranksUpdateThread.class.getName());

	Path crawlDir;
	Map<String, Float> modificationsMap;
	
	public PageranksUpdateThread(Configuration configuration, Path _instanceFolder, Map<String, Float> modifs) {
		super(configuration);
	...
	public void run(){
	    ...
	    try {
	    	...
	    	for (String url : modificationsMap.keySet()) {
	    		System.out.println(url + " - " + CrawlDbMetasMerger.META + " -> " + modificationsMap.get(url));
			}
